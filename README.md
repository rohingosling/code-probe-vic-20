...
Run all commands from the repository root so the relative paths to `roms/` and `build/` resolve. `xvic` must be on `PATH`, or substitute the full path to your VICE install.
